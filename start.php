...
elgg_register_event_handler('init', 'system', 'myspace_init');

// get the current url
$url = "http://" . $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
// puts your elgg site's url into a variable
$elgg = elgg_get_site_url();

// putting these outside of the myspace_init() function due to it not working inside it
if ( fnmatch($elgg . 'profile/*/edit*', $url) || elgg_get_context() == 'reportedcontent' || 
fnmatch($elgg . 'groups/add*', $url) || fnmatch($elgg . 'groups/edit*', $url) ) {
	elgg_unregister_event_handler('init', 'system', 'tinymce_init');
	elgg_unregister_event_handler('init', 'system', 'extended_tinymce_init');
}

function myspace_init() {

	// double entry of these because you can't use variables inside or outside functions
	$url = "http://" . $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
	$elgg = elgg_get_site_url();
	// gets current user logged in
	$user = $_SESSION['user'];
	// gets current view's entity
	$entity = $vars['entity'];
	...
	// replacing the whole friends/friendsof/members view to show as polaroid style 
	...
	// load the necessary libraries for side scroll
	...
	// activity css/js (same setup for the rest)
	if((elgg_get_context() == 'activity')) {
		...
		elgg_register_css('css_activity', 'mod/myspace/css/activity.css');
		elgg_load_css('css_activity');
		elgg_register_js('js_activity', 'mod/myspace/js/activity.js');
		elgg_load_js('js_activity');
		// show geo_location on the river for thewire objects
		elgg_extend_view('river/object/thewire/create', 'myspace/river');
	}

	if((elgg_get_context() == 'profile')) {
		elgg_register_css('css_profile', 'mod/myspace/css/profile.css');
		elgg_load_css('css_profile');
		elgg_register_js('js_profile', 'mod/myspace/js/profile.js');
		elgg_load_js('js_profile');
	}

	// when editing your own profile, my geoPlugin will detect user's location automatically and
	// fill it out if they don't choose to put their location on their own
	// asterisks (*) used for wild cards
	if (fnmatch($elgg . 'profile/*/edit*', $url)) {
		elgg_unregister_js('js_profile');
		elgg_unregister_css('css_profile');
		elgg_unregister_js('jquery.main');
		elgg_unregister_js('jquery.mousewheel.min');
		elgg_register_css('css_editprofile', 'mod/myspace/css/editprofile.css');
	...
	if (elgg_get_context() == 'reportedcontent') {
		elgg_register_css('css_report', 'mod/myspace/css/editprofile.css');
	...
	if (fnmatch($elgg . 'avatar/edit*', $url)) {
		elgg_unregister_js('jquery.main');
		elgg_unregister_js('jquery.mousewheel.min');
		elgg_register_css('css_avatar', 'mod/myspace/css/avatar.css');
	...
	if ((fnmatch($elgg . 'friends*', $url)) || (fnmatch($elgg . 'friendsof*', $url))) {
		elgg_register_css('css_friends', 'mod/myspace/css/friends.css');
		elgg_load_css('css_friends');
		elgg_register_js('js_friends', 'mod/myspace/js/friends.js');
		elgg_load_js('js_friends');
	}

	// who uses collections? removed it and will forward users if they happen to know the url to this
	...
	if (fnmatch($elgg . 'blog/*', $url)) {
		elgg_register_css('css_blog', 'mod/myspace/css/blog.css');
		...
		if (fnmatch($elgg . 'blog/add/*', $url) || fnmatch($elgg . 'blog/edit/*', $url)) {
			...
			elgg_register_css('css_blogaddedit', 'mod/myspace/css/blog-add-edit.css');
		...
		if (fnmatch($elgg . 'blog/view/*', $url)) {
			...
			elgg_unregister_js('jquery.mousewheel.min');
			elgg_unregister_css('css_blog');
			elgg_register_css('css_blogview', 'mod/myspace/css/blog-view.css');
	...
	if (fnmatch($elgg . 'groups*', $url)) {
		elgg_register_css('css_groups', 'mod/myspace/css/groups.css');
		...
		if (fnmatch($elgg . 'groups/add*', $url)) {
			...
			elgg_register_css('css_groups_add', 'mod/myspace/css/groups-add-edit.css');
		...
		if (fnmatch($elgg . 'groups/edit*', $url)) {
			...
			elgg_unregister_css('css_groups');
			elgg_unregister_js('js_groups');
			elgg_register_css('css_groups_edit', 'mod/myspace/css/groups-add-edit.css');
		...
		if (fnmatch($elgg . 'groups/profile*', $url)) {
			elgg_unregister_css('css_groups');
			elgg_unregister_js('js_groups');
			elgg_register_css('css_groups_profile', 'mod/myspace/css/groups-profile.css');
		...
		if (fnmatch($elgg . 'groups/all?filter=discussion*', $url)) {
			elgg_register_css('css_groups_discussion_filter', 'mod/myspace/css/groups-discussion-filter.css');
			elgg_load_css('css_groups_discussion_filter');
		}

		if (fnmatch($elgg . 'groups/invitations*', $url)) {
			elgg_register_css('css_groups_invitations', 'mod/myspace/css/groups-invitations.css');
		...
		if (fnmatch($elgg . 'groups/invite*', $url)) {
			...
			elgg_unregister_js('jquery.mousewheel.min');
			elgg_unregister_js('js_groups');
			elgg_register_css('css_groups_invite', 'mod/myspace/css/groups-invite.css');
		...
		if (fnmatch($elgg . 'groups/requests*', $url)) {
			elgg_register_css('css_groups_requests', 'mod/myspace/css/groups-requests.css');
	...
	if (fnmatch($elgg . 'discussion*', $url)) {
		elgg_register_css('css_discussion', 'mod/myspace/css/discussion.css');
		...
		if (fnmatch($elgg . 'discussion/view*', $url)) {
			...
			elgg_register_css('css_discussion_view', 'mod/myspace/css/discussion-view.css');
		...
		if ((fnmatch($elgg . 'discussion/add*', $url)) || (fnmatch($elgg . 'discussion/edit*', $url))) {
			elgg_unregister_css('css_discussion');
			elgg_unregister_js('js_discussion');
			elgg_register_css('css_discussion_add_edit', 'mod/myspace/css/discussion-add-edit.css');
	...
	if (fnmatch($elgg . 'members*', $url)) {
		elgg_register_css('css_members', 'mod/myspace/css/members.css');
	...
	if (elgg_get_context() == 'messages') {
		elgg_unregister_js('jquery.main');
		elgg_unregister_js('jquery.mousewheel.min');
		elgg_register_css('css_messages', 'mod/myspace/css/messages.css');
	...
	if ((elgg_get_context() == 'settings') || (elgg_get_context() == 'notifications')) {
		elgg_unregister_js('jquery.main');
		elgg_unregister_js('jquery.mousewheel.min');
		elgg_register_css('css_settings', 'mod/myspace/css/settings.css');
...
// grabs IP address for the geoPlugin to trace it
